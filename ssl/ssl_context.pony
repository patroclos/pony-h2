use "files"
use "lib:alpn"
use @server_use_protos[None](ctx: Pointer[_SSLContext] tag, protocols: Pointer[Pointer[U8] tag] tag, numProtos: USize)
...
class val SSLContext
  ...
  fun ref set_alpn_protos(protos: Array[String] iso) =>
    let size = protos.size()
    let cprotos = Array[Pointer[U8] tag](protos.size())

    for p in (consume protos).values() do
      cprotos.push(recover tag p.cstring() end)
    end

    @server_use_protos(_ctx, cprotos.cpointer(),size)
